...
func unwrapOmittable(t types.Type) (types.Type, bool) {
	...
	if named.Origin().String() != "github.com/OldBigBuddha/gqlgen/graphql.Omittable[T any]" {
		return t, false
	}
...
func (b *Binder) TypeReference(schemaType *ast.Type, bindTarget types.Type) (ret *TypeReference, err error) {
	...
	for _, model := range b.cfg.Models[schemaType.Name()].Model {
		...
		if values := b.enumValues(def); len(values) > 0 {
			...
			ref.IsContext = code.Unalias(t.(*types.Signature).Results().At(0).Type()).String() == "github.com/OldBigBuddha/gqlgen/graphql.ContextMarshaler"
			ref.Marshaler = fun
