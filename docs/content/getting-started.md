...
import (
	_ "github.com/OldBigBuddha/gqlgen"
)
```
...
```shell
go get -d github.com/OldBigBuddha/gqlgen@VERSION
```
...
And run `go run github.com/OldBigBuddha/gqlgen generate`.

>
> If you run into this error `package github.com/OldBigBuddha/gqlgen: no Go files` while executing the `generate` command above, follow the instructions in [this](https://github.com/OldBigBuddha/gqlgen/issues/800#issuecomment-888908950) comment for a possible solution.
...
```go
//go:generate go run github.com/OldBigBuddha/gqlgen generate
```
